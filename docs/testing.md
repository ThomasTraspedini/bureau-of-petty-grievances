...
At version `0.19.0`, use Node.js 24, install dependencies and the Playwright Chromium, Firefox, and WebKit engines, then run:
...
The canonical workflow includes repository and prototype validation, formatting, linting and dependency-direction rules, strict type checking, focused Vitest component and locale tests, a production build, and Playwright checks. The complete established browser suite and reviewed pixel references run in Chromium. A bounded compatibility contract additionally runs in Firefox and WebKit and covers the essential provider-free filing-to-public-consultation journey, explicit locale routing, mobile and representative desktop viewports, refresh recovery, keyboard access, reduced motion, sharing fallback, and axe-core accessibility. A Chromium recovery contract injects one-shot determination-completion, publication, and consultation failures through the production server-action boundary, then proves state preservation, accessible localized feedback, exact retry, and successful continuation. A separate Chromium-only localization stress contract renders proportionally expanded interface copy at 320 pixels across filing, determination, publication, public consultation, sharing fallback, owner controls, standard access, and unavailable evaluation access. It asserts document and surface containment, essential-action bounds, accessible names, and axe-core results. Dedicated route contracts for Italian, French, German, Spanish, and Brazilian Portuguese verify localized metadata and document language, then complete filing, determination refresh, publication, public-record consultation, and axe-core accessibility in each language. CI invokes the same workflow on Node.js 24.

## Production application verification

Vitest protects the typed locale allowlist, catalog pseudo-localization, placeholder preservation, explicit route navigation, honest shell content, the valid deterministic representative determination, department switching, all enabled department fact variants, server-ready normalization, deterministic assessment, conservative content boundaries, 30-day draft lifecycle, legacy migration, transient determination lifecycle, storage tampering, public-record identity and lifecycle rules, PostgreSQL persistence, recovery, and component validation. Playwright exercises the production build rather than a test-only rendering path.
...
Determination-language tests cover matching filing and assessment commands, schema and department editorial-policy versions, grounding, locale and disposition, factual-number and lexical anchors, length, hostile free text, restricted claims, tone, and non-binding remedy constraints. Exhaustive supported-combination checks prove the English fallback for all enabled offences, severity bands, impacts, mitigations, and relationship contexts. Locale-specific fixtures for Italian, French, German, Spanish, and Brazilian Portuguese cover all four departments, grounded fallback validity, provider prompt selection and redaction, hostile-output rejection, generation fallback, and localized snapshot validation. Recursive catalog tests prove English key and placeholder parity for every supported language. Independent curated Chronology fixtures establish representative Bureau language; Digital Conduct, Domestic Affairs, and Social Planning exercise their complete deterministic fallbacks and stricter privacy and safety boundaries.
...
The filing browser journey covers all phases from a blank alias through an issued determination, explicit department selection, direct correction back to review, draft and determination refresh recovery, suppression of false recovery notices during internal navigation, exclusion of rejected witness text, unavailable direct navigation, keyboard radio behavior, reduced motion, axe-core accessibility, and mobile and desktop visual references. Complete Digital Conduct, Domestic Affairs, and Social Planning paths render their distinct evidence grammars, expose exact whole-number ranges without wrapping unit labels, publish records, verify bounded share metadata, and prove aliases and excluded facts are absent from preview markup. The journey continues through informed publication, public rendering, categorical reporting, private owner recovery, unpublishing, restoration, permanent deletion, and non-enumerating unavailable rendering. Session tests reject expired or altered facts, assessment, prose, identity, and versions and restore every enabled department snapshot. The recovery contract proves that a failed determination retains the safe filing and original idempotency key, a failed publication exposes no public address and leaves the transient determination available, and a failed consultation retains its selection and anonymous participation key before one successful response is recorded. Browser controls remain disabled until validated state has loaded, and the browser test server explicitly clears provider credentials so ordinary verification cannot make a paid call. Failure injection is additionally gated by a test-server-only setting and a strict request instruction; either boundary being absent leaves it inert.
...
Visual references use the self-hosted production fonts, disable motion, and remain a single Chromium contract with a one-percent pixel-difference tolerance for platform rasterization. Full-page desktop references protect the dedicated representative determination and the Social Planning register; a separate mid-width revision-impact case protects the register's row layout and long planning-context values. Two focused 320-pixel references preserve the first viewport of the expanded landing and public-record surfaces. The pseudo-localized narrow-layout journey also checks the example page before filing. Firefox and WebKit use behavioral, viewport, and accessibility assertions rather than separate screenshots. Reference images change only after manual inspection at the affected viewport.
