...
The application foundation produces a full Node.js server build on Node.js 24. This preserves the framework capabilities required for protected server actions and dynamic public records. The approved production stack is Netlify Free with Neon Free PostgreSQL and the production Netlify HTTPS domain; no custom domain is required. Netlify’s Next.js adapter supplies the server runtime. Persistence uses ordinary PostgreSQL without Supabase services or SDKs. See [deployment decision](decisions/0025-production-deployment-stack.md).

A deployed instance must set `BUREAU_PUBLIC_ORIGIN` to its bare public HTTPS origin. This non-secret setting is the sole authority for canonical record links, share payloads, and social-image addresses; forwarded request headers are intentionally ignored. Missing or invalid production configuration makes public records unavailable without exposing internal configuration details. Single-device development and automated tests use an explicit loopback origin. Cross-device publication and sharing verification require a trusted HTTPS origin reachable from every device; set the same canonical origin on the server. A LAN HTTP origin is rejected, and localhost links do not address the host from another device.

The rendered application has no required secrets because provider absence selects the complete deterministic fallback. When configured, the filing server action invokes the adapter using `OPENAI_API_KEY` and a configurable `BUREAU_OPENAI_MODEL` whose default is `gpt-5.6-terra`; credentials remain in ignored local or deployment configuration. Production font files are bundled locally. CI and local verification explicitly provide no provider credential, make no provider calls, and use the same repository command after installing the Playwright Chromium, Firefox, and WebKit engines. Chromium owns the complete regression and visual suite; a bounded essential journey runs in all three engines.
...
The language seam sends a matching assessment command to the OpenAI Responses API only when deliberately invoked with configuration. Requests select the matching English, Italian, French, German, Spanish, or Brazilian Portuguese policy by locale and department, disable response storage, exclude the respondent alias and exact clock time, bound output, use an 18-second SDK timeout, and disable SDK retries. The product owns at most one retry and converts provider responses, refusals, and failures into typed outcomes. Raw provider errors and invalid text are discarded; a complete locale-owned fallback remains available for every valid command in all four departments. Unsupported locale input is rejected at the route and request boundaries; English is never used as a silent substitute for a valid supported-locale filing.

## Netlify and Neon deployment runbook

1. Create a Netlify Free project from this repository. Use the repository root, `npm run build`, and `.next` publish directory as declared in `netlify.toml`. Use the automatic Next.js adapter, not a static export or an `npm start` process. Set `AWS_LAMBDA_JS_RUNTIME=nodejs24.x` in the Netlify UI before building; this runtime selection cannot be supplied through `netlify.toml`.
2. Create a Neon Free PostgreSQL project, choosing a region near the Netlify function region. Use the Neon connection dialog’s pooled connection string (the `-pooler` hostname), preserving its TLS parameters. Enter it directly as production `DATABASE_URL` in Netlify; never place it in source, chat, logs, or a browser-visible variable. The database role must be able to apply the application schema. No Supabase migration or SDK setup is needed.
3. Set production `BUREAU_PUBLIC_ORIGIN=https://<assigned-site-name>.netlify.app` after the site name is known. Set a random server-only `BUREAU_NETWORK_HMAC_SECRET` of at least 32 characters directly in platform configuration. Keep `BUREAU_ANALYTICS_ENABLED=false`. Omit embedded-database and development-origin settings. Leave the OpenAI key unset until provider privacy and budget controls are verified.
4. Keep production credentials exclusive to production. Disable Deploy Previews and branch deploys before configuring secrets if the account cannot isolate their environment values. Any later preview must use a separate database and credentials. Do not expose production access through deploy previews.
5. Verify the actual Netlify adapter’s trusted `X-Forwarded-For` chain before setting `BUREAU_TRUSTED_PROXY_HOPS`: the existing algorithm selects from the right after skipping that many entries. Check spoofed incoming headers and direct-origin bypass without logging raw addresses. Do not treat the local example value as verified hosting evidence. Evaluator exchange and paid generation are not qualified until this check passes.
6. Deploy and verify schema version 8, filing/publication, canonical links and social images, consultation, owner lifecycle, and persistence across redeploy. Let Neon become idle, then verify an existing record and a new write resume successfully without owner intervention. Verify timeout recovery and simultaneous cold starts; the existing transaction migration has not yet been qualified on this hosted stack.
7. Review the actual Free-plan quotas, backup/restore and retention settings in both dashboards, and record operational limits before evaluation. Free tiers do not establish an uptime guarantee. Verify the hosted function timeout accommodates the bounded provider retry before enabling provider calls. Then verify access, fallback/refunds, budget and kill switch, and run the full release gate once on the stabilized candidate.

Authenticated account creation, repository authorization, and secret entry happen in the provider dashboards. Operators do not paste credentials into chat. Until hosted checks are recorded, the stack is approved but deployment is unverified.

Platform references: [Netlify Next.js adapter](https://docs.netlify.com/build/frameworks/framework-setup-guides/nextjs/overview/), [Node.js function runtime configuration](https://docs.netlify.com/build/functions/configuration/), [Neon pooling](https://neon.com/docs/connect/connection-pooling), and [Neon scale to zero](https://neon.com/docs/introduction/scale-to-zero).
...
Direct non-production requests may omit `X-Forwarded-For`; those requests share one development-only network pseudonym so evaluator links remain testable from trusted devices on a local network. Production never applies this fallback and continues to fail closed without a trustworthy proxy-appended address.
...
- Domestic Affairs accepts only bounded filer-entered quantities, correction measures, and recurrence. It has no provider or application permission to receive photos, sensor data, home maps, room names, household surveillance, or external verification; a deployment must not add such access without a new accepted privacy and safety decision.
- Social Planning accepts only bounded filer-entered planning counts and intervals. It has no provider or application permission to access calendars, messages, contacts, locations, invitations, guest lists, attendance records, social networks, or external observation; a deployment must not add such access without a new accepted privacy and safety decision.
...
For an explicitly designated release/deployment task (not routine implementation or documentation):

- run `./scripts/validate-repository.sh --full` once on the release candidate;
